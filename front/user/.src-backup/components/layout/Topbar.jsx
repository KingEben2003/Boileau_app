...
function FriendsPanel({ onClose, onSectionChange, onChallengeFriend, onCountChange }) {
  ...
  return (
    <motion.div
      ...
      className="absolute right-0 top-full mt-3 w-80 bg-[#0d1117] border border-white/10 rounded-2xl shadow-2xl shadow-black/60 overflow-hidden z-50"
    >
      {/* Tabs */}
...
/* ─── TopBar principal ────────────────────────────────────────────── */
export default function TopBar({ toggleSidebar, onSectionChange, onChallengeFriend }) {
  ...
  return (
    <header className="h-20 bg-white/5 backdrop-blur-md flex items-center px-6 md:px-10 justify-between border-b border-white/10">
      {/* Gauche */}
      <div className="flex items-center gap-6">
        <button
          onClick={toggleSidebar}
          className="md:hidden p-2.5 rounded-xl bg-white/5 hover:bg-white/10 text-gray-400 transition-all"
        >
          <FiMenu size={20} />
        </button>

        <div className="hidden md:flex items-center gap-3 px-4 py-2 bg-white/5 border border-white/10 rounded-xl w-80 focus-within:border-indigo-500/50 transition-all duration-300">
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="relative" ref={friendsRef}>
          <motion.button
            ...
            className="p-2.5 rounded-xl bg-white/5 hover:bg-white/10 text-gray-400 hover:text-indigo-400 transition-all relative"
            title="Amis"
          >
            <FiUsers size={20} />
            {pendingCount > 0 && (
        ...
        <button
          ...
          className={`p-2.5 rounded-xl bg-white/5 hover:bg-white/10 transition-all relative group ${
          ...
          <AnimatePresence>
            {dropdownOpen && (
              <motion.div
                ...
                className="absolute right-0 top-full mt-3 w-52 bg-[#0d1117] border border-white/10 rounded-2xl shadow-2xl shadow-black/40 overflow-hidden z-50"
              >
