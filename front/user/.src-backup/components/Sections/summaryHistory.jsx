    ...
    return (
      <div className="w-full max-w-2xl mx-auto">
        <div className="glass-card rounded-[2rem] p-8 text-center">
          <div className="w-20 h-20 mx-auto mb-5 rounded-full bg-gradient-to-br from-green-500 to-pink-500 flex items-center justify-center">
            <span className="text-3xl font-black text-white">{score.pct}%</span>
          </div>
              ...
              return (
                <div key={q.id} className={`p-4 rounded-2xl border ${ok ? "bg-emerald-500/10 border-emerald-500/20" : "bg-red-500/10 border-red-500/20"}`}>
                  <div className="flex gap-3">
                    ...
                    <div>
                      <p className="text-sm font-semibold text-white mb-1">{q.question_text}</p>
                      <p className="text-xs text-gray-400">Votre réponse : {ua || "—"}</p>
                      {!ok && <p className="text-xs text-emerald-400 mt-0.5">Bonne réponse : {q.correct_answer}</p>}
                    </div>
                  </div>
                </div>
              );
  ...
  return (
    <div className="w-full max-w-2xl mx-auto">
      ...
      <div className="glass-card rounded-[2rem] p-7">
        ...
        <p className="text-base font-bold text-white mb-5">{q.question_text}</p>

        <div className="space-y-2.5 mb-7">
          {(q.type === "open" || (quiz.type === "open" && !q.options?.length)) ? (
            <textarea
              value={answers[q.id] || ""}
...
function QuizzesTab({ documentId, document }) {
  ...
  const handleGenerate = async () => {
    ...
    setGenError("");
    try {
      await generateQuiz(documentId, { type: genType, numberOfQuestions: genCount, difficulty: "medium" });
      setShowForm(false);
      await loadQuizzes();
    } catch (err) {
    ...
    <div className="space-y-5">
      ...
      <div className="glass-card rounded-[2rem] overflow-hidden">
        <button
          ...
          className="w-full flex items-center justify-between px-6 py-4 text-sm font-bold text-gray-300 hover:text-white disabled:opacity-40 disabled:cursor-not-allowed transition-colors group"
        >
          <span className="flex items-center gap-2">
            <FiPlus size={16} className="text-pink-400" />
            Générer un nouveau quiz
            <span className="text-[10px] font-normal text-gray-600 ml-1">
              ({quizzes.length}/{QUIZ_LIMIT})
            </span>
          </span>
          {showForm ? <FiChevronUp size={15} className="text-gray-500" /> : <FiChevronDown size={15} className="text-gray-500" />}
        </button>
            ...
            >
              <div className="px-6 py-5 space-y-5">
                ...
                <div>
                  ...
                  <input
                    type="range" min="1" max="20" step="1" value={genCount}
              ...
              <div className="flex items-center justify-between gap-4">
                ...
                {isDone ? (
                  ...
                  <motion.button
                    ...
                    whileTap={{ scale: 0.95 }}
                    onClick={() => setActiveQuiz(quiz)}
                    className="flex items-center gap-2 px-4 py-2.5 bg-gradient-to-r from-pink-600 to-purple-600 hover:from-pink-500 hover:to-purple-500 text-white rounded-xl text-xs font-bold transition-all flex-shrink-0 shadow-lg shadow-pink-500/20"
                  >
      ...
      >
        <div className="flex items-center justify-between mb-6">
          ...
          <div className="flex items-center gap-2">
            <button
              onClick={handleCopy}
              className="flex items-center gap-2 px-4 py-2 bg-white/5 hover:bg-white/10 border border-white/10 text-gray-300 hover:text-white rounded-xl transition-all text-sm font-semibold"
            >
            ...
            <button
              ...
              className="flex items-center gap-2 px-4 py-2 bg-indigo-600 hover:bg-indigo-500 text-white rounded-xl transition-all text-sm font-bold"
            >
        ...
        <div className="flex flex-wrap items-start gap-4 mb-8">
          ...
          <div className="flex items-center gap-1.5 text-xs text-gray-500">
            <FiClock size={11} />{formatDate(selectedSummary.created_at)}
          </div>
        </div>
  ...
  return (
    <div className="w-full">
      ...
      >
        ...
        <h3
          className="font-black tracking-tight bg-gradient-to-r from-indigo-400 via-purple-400 to-pink-400 bg-clip-text text-transparent leading-tight"
        ...
        <div className="w-full">
          <div className="flex items-center justify-between mb-5">
            ...
            <button
              ...
              disabled={loading}
              className="flex items-center gap-2 text-sm px-4 py-2 bg-white/5 hover:bg-white/10 border border-white/10 text-gray-300 hover:text-white rounded-xl transition-all font-semibold"
            >
            ...
            {summaries.map((summary, i) => {
              ...
              return (
                <motion.div
                  ...
                  animate={{ opacity: 1, y: 0 }}
                  transition={{ duration: 0.25, delay: i * 0.05 }}
                  className="glass-card rounded-2xl p-5 hover:border-indigo-500/30 transition-all"
                >
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex items-start gap-4 flex-1 min-w-0">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs text-gray-500 line-clamp-2 leading-relaxed">{summary.content.slice(0, 120)}…</p>
                      </div>
                    </div>
