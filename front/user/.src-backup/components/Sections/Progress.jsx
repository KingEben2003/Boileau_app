        ...
        <motion.div
          ...
          className={`relative p-4 rounded-2xl border text-center cursor-pointer transition-all ${
            item.unlocked
              ...
              : "bg-white/3 border-white/10 hover:border-white/20 opacity-60"
          }`}
        >
    ...
    <div>
      {!hasAnyData ? (
        <div className="flex flex-col items-center justify-center h-44 gap-3">
          ...
          <p className="text-sm text-gray-500">Aucune donnée — faites des quiz pour voir votre courbe.</p>
        </div>
      ) : (
        <div className="relative select-none">
          <svg viewBox={`0 0 ${W} ${H}`} className="w-full" style={{ height: 190 }}>
            ...
            {gridVals.map((v) => (
              <g key={v}>
                ...
                />
                <text x={PL - 5} y={Y(v) + 3.5} textAnchor="end" fontSize="9" fill="#4b5563">{v}</text>
              </g>
            ))}
            ...
            {globalAvg > 0 && (
              <>
                ...
                />
                <text x={W - PR + 3} y={Y(globalAvg) + 3.5} fontSize="8" fill="#6366f1">moy</text>
              </>
            )}
            ...
            {/* Hover areas + data points */}
            {pts.map((p) => (
              <g key={p.i}
                onMouseEnter={() => setHover(p)}
                onMouseLeave={() => setHover(null)}
                style={{ cursor: p.score !== null ? "default" : "default" }}
              >
                {/* Wide invisible hover zone per column */}
                <rect
                  x={X(p.i) - CW / (N * 2)}
                  y={PT}
                  width={CW / N}
          ...
          <div className="mt-1.5 h-7 flex items-center justify-center">
            {hover?.score != null ? (
              ...
              <p className="text-xs text-gray-600">Survolez le graphique pour voir le score exact</p>
            )}
          </div>
        </div>
      )}
  ...
  return (
    <div className="w-full space-y-8 pb-10">
      <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }}>
        <h2 className="text-2xl font-black text-white tracking-tight">Ma Progression</h2>
        <p className="text-gray-500 text-sm mt-1">Performances, gamification et révisions espacées</p>
      </motion.div>

      {error && (
        <div className="p-4 bg-red-500/20 border border-red-500/50 rounded-xl text-red-300 text-sm">{error}</div>
      )}
        ...
        {loading
          ...
          : statCards.map((card, i) => (
              <motion.div
                ...
                className={`glass-card rounded-2xl p-5 border ${card.border} ${card.bg} flex items-center gap-4`}
              >
                <div className={`p-3 rounded-xl bg-gradient-to-br ${card.grad} text-white shadow-lg flex-shrink-0`}>
                  {card.icon}
                </div>
                <div className="min-w-0">
                  <p className="text-xs text-gray-500 uppercase tracking-widest leading-tight">{card.label}</p>
                  <p className={`text-2xl font-black ${card.text} leading-tight`}>{card.value}</p>
                </div>
              </motion.div>
            ))}
      </div>
      ...
      >
        <div className="flex items-center justify-between mb-4">
          <div>
            ...
            <p className="text-3xl font-black text-purple-400 leading-tight">Niveau {level}</p>
          </div>
        ...
        <p className="text-xs text-gray-500 mt-2">{xpInLevel} / 100 XP · encore {xpToNext} XP pour le niveau suivant</p>
      </motion.div>
      ...
      >
        <div className="flex items-center gap-2 mb-1">
          ...
          <h3 className="text-base font-bold text-white">Échelle de niveaux</h3>
        </div>
        <p className="text-xs text-gray-500 mb-5">Votre parcours de Novice à Dieu — niveau actuel : <span className="text-purple-400 font-bold">{LEVEL_NAMES[(level - 1) % 10]}</span></p>
      ...
      >
        <div className="flex items-center gap-2 mb-1">
          ...
          <h3 className="text-base font-bold text-white">Défis quotidiens</h3>
        </div>
      ...
      >
        <div className="flex items-center gap-2 mb-1">
          ...
          <h3 className="text-base font-bold text-white">Faiblesses par thème</h3>
        </div>
      ...
      >
        <div className="flex items-center gap-2 mb-1">
          ...
          <h3 className="text-base font-bold text-white">Récompenses cosmétiques</h3>
        </div>
      ...
      {dashboard?.benchmark && (
        <motion.div
          ...
          className="glass-card rounded-2xl p-6 border border-indigo-500/20 flex flex-col sm:flex-row items-start sm:items-center gap-6"
        >
          ...
          {dashboard.projected_next_score != null && (
            <div className="flex-shrink-0 text-right">
              ...
              <p className="text-2xl font-black text-pink-400">{Number(dashboard.projected_next_score).toFixed(0)}%</p>
      ...
      >
        <div className="flex items-center gap-2 mb-1">
          ...
          <h3 className="text-base font-bold text-white">Documents à réviser</h3>
        </div>
        ...
        {loading ? (
          ...
          <div className="space-y-3">
            {srsItems.map((item) => (
              <div
                key={item.id}
                className="flex items-center gap-4 p-4 bg-cyan-500/5 border border-cyan-500/20 rounded-xl"
              >
                <div className="flex-1 min-w-0">
                ...
                <button
                  ...
                  className="flex-shrink-0 flex items-center gap-1.5 px-3 py-1.5 bg-cyan-500/20 hover:bg-cyan-500/30 border border-cyan-500/40 text-cyan-300 rounded-xl text-xs font-bold transition-all disabled:opacity-50"
                >
                  {reviewingId === item.id ? (
      ...
      >
        <div className="flex items-center justify-between mb-1">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-base font-bold text-white">Analyse IA des faiblesses</h3>
          </div>
          <button
            onClick={handleAnalyzeWeaknesses}
            disabled={weaknessLoading}
            className="flex items-center gap-2 px-4 py-2 bg-amber-500/20 hover:bg-amber-500/30 border border-amber-500/40 text-amber-300 rounded-xl text-xs font-bold transition-all disabled:opacity-50"
          >
            {weaknessLoading ? (
      ...
      {dashboard?.weakest_documents && dashboard.weakest_documents.length > 0 && (
        <motion.div
          ...
          className="glass-card rounded-2xl p-6 border border-white/10"
        >
          <h3 className="text-base font-bold text-white mb-1">Documents les plus faibles</h3>
